...
export const fieldValidationDefinition: Definition = {
  campaignCd: yup.string().label('campaignCd').max(15).halfWidthOnly(),
  campaignName: yup.string().label('campaignName').max(30).fullAndHalfWidth(),
  discountAmount: yup.string().label('discountAmount').max(6).numberOnly(),
  contractQuantityLowLimit: yup.string().label('contractQuantityLowLimit').max(4).numberOnly(),
  contractQuantityHighLimit: yup.string().label('contractQuantityHighLimit').max(4).numberOnly(),
  ContractAfterMonth: yup.string().label('ContractAfterMonth').max(4).numberOnly(),
  approvalDocumentId: yup.string().label('approvalDocumentId').max(20).halfWidthOnly(),
  optionCampaignCd: yup.string().label('optionCampaignCd').max(15).halfWidthOnly(),
  optionCampaignName: yup.string().label('optionCampaignName').max(30).fullAndHalfWidth(),
  optionDiscountAmount: yup.string().label('optionDiscountAmount').max(6).numberOnly(),
  optionContractQuantityLowLimit: yup.string().label('optionContractQuantityLowLimit').max(4).numberOnly(),
  optionContractQuantityHighLimit: yup.string().label('optionContractQuantityHighLimit').max(4).numberOnly(),
  optionContractAfterMonth: yup.string().label('optionContractAfterMonth').max(4).numberOnly(),
  optionApprovalDocumentId: yup.string().label('optionApprovalDocumentId').max(20).halfWidthOnly(),
};