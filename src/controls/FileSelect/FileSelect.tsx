...
export const FileSelect = <T extends FieldValues> (props: FileSelectProps<T>) => {
  const { name, setValue, label, labelPosition } = props;
  ...
  return (
    <InputLayout label={label} labelPosition={labelPosition}>
      <Box>
        <Stack direction='row' spacing={3} alignItems='center' marginBottom={2}>
          ...
          <Typography>{acceptedFiles.length > 0 ? acceptedFiles[0].name : '未選択'}</Typography>
        </Stack>
        <Box sx={{ border: 1, padding: 2, backgroundColor: '#F5F5F5', minHeight: 300 }} {...getRootProps()}>
